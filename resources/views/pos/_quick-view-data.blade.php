...
<div class="modal-body">
    <div class="">
        ...
        <div class="d-flex align-items-center justify-content-center pvl41 active">
            <img class="img-responsive pvl42"
                 src="{{asset('storage/images/medicine/'.$product->image.'')}}"
            ...
            <form id="add-to-cart-form" class="mb-2">
                ...
                <div class="position-relative {{Session::get('direction') === "rtl" ? 'ml-n4' : 'mr-n4'}} mb-3">
                    ...
                    @php
                        $qty = 0;
                        if(session('stock') == 'emergency-stock'){
                         $batch = \App\Models\Batch::where('emergency_stock_id', $product->id)->where('shop_id', Auth::user()->shop_id)->where('qty', '>', 0)->where('expire', '>=', date('Y-m-d'))->get();

                        }else{
                           $batch = \App\Models\Batch::where('medicine_id', $product->id)->where('shop_id', Auth::user()->shop_id)->where('qty', '>', 0)->where('expire', '>=', date('Y-m-d'))->get();
                        }
                            ...
                            <select class="pvl50" name="batch" id="select">
                                @foreach ($batch as $key => $option)
                                    <option id="{{ $option->id }}-{{ $option->id }}" value="{{ $option->id }}"
                                            @if($key == 0) selected
                                            @endif autocomplete="off">{{trans('Expired')}} {{ date('d F, Y', strtotime($option->expire)) }}
                                        ({{trans('Stock')}} : {{ $option->qty }}) (MRP : {{ number_format($option->price,2) }} TK)
                                    </option>
                                @endforeach
                            </select>
                        </div>
